---
title: "自动驾驶视觉感知算法实习复盘：从点云语义建图到 APA/MPA 指标"
published: 2025-08-30
updated: 2025-08-30
draft: false
description: "整理我在武汉光庭技术股份有限公司视觉感知算法实习期间参与的 AVM、点云语义建图、APA/MPA 指标计算、PCL 预处理和 CloudCompare 可视化工作。"
...
这篇是我把当时的自动驾驶视觉感知算法实习内容重新整理后的技术复盘，时间范围是 2025.03 到 2025.08。

这段经历和后面的车载终端测试开发是两条不同线：这里更偏智能驾驶视觉感知算法，主要围绕 AVM 全景环视、语义分割、点云语义建图、APA/MPA 指标计算和三维点云映射。

我下面只写能公开表达的技术方向、工作拆解和个人收获，不展开内部实现细节。
...
我负责的一部分工作，是围绕亚像素级角点检测做算法开发和效果优化。普通像素级角点能给出大致位置，但在拼接场景里，几个像素的误差就可能带来明显接缝、错位或者局部拉伸。
...
这份资料里我最重视的一块，是 APA 和 MPA 的计算与可视化。
...
对我来说，这一步的意义不是“会算指标”，而是能判断模型是不是在往正确方向走。只看单次结果很容易误判，指标曲线才能更稳定地反映模型收敛情况。

## 五、两张结果图：我保留下来的可视化证据

这两张图是我从资料里保留下来的结果图，分别对应阈值处理和对比度增强后的效果。它们更像是预处理和可视化的中间证据，不是最终结论，但很适合说明我当时是怎么排查和对比结果的。
...
## 六、代码拆解：我当时是按这个思路往下做的

这段实习给我的感受是，智能驾驶里的视觉算法不能只看单个模型或单张图片效果。它最后要进入一条连续链路：
...
报告里我先把 PCD 文件的读写、滤波和下采样流程理顺了。工程上最常见的需求不是“直接喂给模型”，而是先把原始点云清洗到可用状态。
...
如果坐标系不统一，后面的语义建图和结果分析都会失真。所以我会先把世界坐标、相机坐标和点云坐标理顺，再做映射。
...
我后来会把 APA / MPA 曲线和类别结果固定成图，而不是只看一遍终端输出。这一步很重要，因为图一旦稳定，后续复盘就能直接用。
...
这段实习让我把计算机视觉、点云处理、三维几何和 C++ 工程开发真正接到智能驾驶场景里。
...
## 八、非技术部分：这段实习也让我补了别的东西

报告里还有一部分我后来也特别认同：技术之外的东西，最后一样会影响项目推进。
...
这也是我后来回头整理这份资料的原因：它不只是一次实习记录，更像是我第一次把“二维视觉结果”真正往“三维工程链路”里推进。
