...
这篇记录的是我在武汉市测绘研究院算法实习期间参与的外立面缺陷检测科研项目中的人工复核链路。
...
所以我把 LabelMe 集成到检测系统里，让用户可以直接对模型检测结果进行修改和补充。
...
我没有在 PyQt 里重写完整标注工具，而是把 LabelMe 作为外部复核工具接入。这样开发成本更低，也更稳定。
...
我的处理方式是：复核文件统一放在项目目录下，由系统生成和读取，不依赖用户手动移动文件。
...
LabelMe 作为外部工具接入后，打包发布时需要准备它的运行环境。这个问题我在 PyInstaller 打包文章里单独复盘。
