---
title: "Robocon 仿生四足机器人视觉模块复盘：OpenCV 赛道识别、路径中线和现场调参"
published: 2024-07-09
updated: 2024-07-09
draft: false
description: "复盘我在 Robocon 仿生四足机器人项目中负责的视觉模块：相机输入、ROI 裁剪、阈值分割、轮廓筛选、中线拟合、偏差输出和现场调参经验。"
...
这篇文章整理的是我在 Robocon 仿生四足机器人项目中负责的视觉模块。项目整体涉及机械结构、运动控制、步态、通信和现场策略，但我这里主要复盘 **视觉方向**：如何从相机画面中稳定提取赛道 / 引导线信息，并把结果转成后续控制模块可以使用的偏移量、方向角和置信度。

为了适合公开展示，下面不会展开完整工程源码，也不会把运动控制部分写成自己的工作。文中的代码片段是按照项目逻辑重建出来的核心示例，重点用于说明图像处理链路和调试思路。

![BeeDog 四足机器人平台](/blog-assets/beedog-quadruped-robot.png)

> 资料中的 BeeDog 四足机器人平台图。视觉模块是在这类四足平台上服务路径感知和赛前联调，不展开机械和步态控制细节。
...
资料里包含 BeeDog 四足平台、ROS 源码包以及 Astra 相机相关驱动。ROS 包中有 `ros_astra_camera`，可提供 RGB / 深度图像、相机参数和点云话题；其中 RGB 启动文件里给出的输入配置是 640 x 480、30 fps。公开复盘时可以把它理解为视觉输入链路的一种可选支撑。
...
这里的比例不是固定真理，需要根据相机安装角度调整。我的经验是：
...
HSV 版本更适合目标线颜色固定的情况。实际调参时我会把 `lower` 和 `upper` 写到配置里，现场根据灯光快速切换。

## 五、轮廓筛选：不要只找最大轮廓

很多入门写法会直接找最大轮廓，但比赛现场不一定可靠。比如地面反光、边框、贴纸、背景物体都可能形成大块区域。我的做法是给轮廓加几层约束：
...
找到候选路径区域后，需要把它转成中线、偏移量和方向角。这里可以用轮廓点拟合直线，也可以按行扫描提取中心点。我的做法更偏工程稳定性：先从二值图里按行取中心点，再做直线拟合。
...
视觉模块不应该直接替控制模块做复杂决策。我更倾向于只输出结构化结果，让控制侧根据策略使用：
...
如果接 ROS，可以把它发布成自定义消息，也可以先用简单 JSON / 串口协议联调。公开展示时不需要把通信协议写得过满，重点是说明视觉模块输出了什么、为什么这样输出。

## 九、现场调参经验

这类项目最容易出问题的地方不是代码能不能跑，而是现场环境和实验环境不一致。我整理过几条比较实用的调参经验：
...
这段 Robocon 四足机器人视觉模块给我的收获主要有三点：
...
如果用一句话总结这段项目：我负责的不是“让机器人会走”，而是让机器人在视觉层面尽可能稳定地知道前方路径在哪里，并把这个信息以低延迟、可解释的方式交给后续模块。
