---
title: "OSGB 实景三维模型加载与 OSGViewer 工程记录"
...
实际处理时，需要把模型目录整体迁移，并在加载前检查 Data、Tile、Texture 等路径是否完整。
...
处理原则是：三维模块独立封装，PyQt 只负责放置和调用，不把 viewer 生命周期散落到多个界面类里。
