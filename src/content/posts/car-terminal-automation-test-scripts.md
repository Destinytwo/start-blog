...
updated: 2025-10-15
draft: false
description: "整理我在英泰斯特车载终端测试开发实习中用到的自动化回归思路：休眠监听、CAN 唤醒、网络恢复、SSH 验证和结果记录。"
image: "/blog-assets/car-terminal-test-cover.svg"
tags: ["自动化测试", "英泰斯特", "回归清单", "TBOX", "CANFD", "ECALL"]
category: "车载测试项目"
author: "Funny"
---
...
这篇只整理我在英泰斯特车载终端测试开发实习中用到的自动化回归思路，重点是把“休眠 - 唤醒 - 网络恢复 - 外网验证”这条链路讲清楚。

这属于英泰斯特测试开发经历里的一个细分场景。这里讨论的是测试开发脚本：前置条件怎么查、日志怎么采、结果怎么判，以及失败时怎么留下可追溯证据。
...
我按实习期间实际验证过的链路，把自动化脚本拆成几个模块来讲。为了避开项目内部细节，下面的代码保留的是结构和判断方式，不展开公司内部字段。
...
对应到英泰斯特这段车载终端测试开发实习，我会把它拆成下面六个部分。
