...
export class SwupHooksManager {
	...
	private registerContentReplaceHook(): void {
		window.swup!.hooks.on("content:replace", () => {
			this.clearCache();
			this.setCardSwapState("entering");
	...
	private registerPageViewHook(): void {
		window.swup!.hooks.on("page:view", () => {
			...
			window.setTimeout(() => this.clearCardSwapState(), 620);
		});
	}
	...
	private setCardSwapState(state: "leaving" | "entering"): void {
		const root = document.documentElement;
		root.classList.toggle("funny-cards-leaving", state === "leaving");
		root.classList.toggle("funny-cards-entering", state === "entering");
	}

	private clearCardSwapState(): void {
		document.documentElement.classList.remove(
			"funny-cards-leaving",
			"funny-cards-entering",
		);
	}
	...
	private syncMainContentPosition(isHomePage: boolean): void {
		...
		const fullscreenOffset = isHomePage
		...
		if (mode === "fullscreen") {
			if (mainContentWrapper.classList.contains("funny-page-sheet")) {
				bannerWrapper?.classList.remove("mobile-hide-banner");
				mainContentWrapper.classList.remove("mobile-main-no-banner", "no-banner-layout");
				mainContentWrapper.classList.toggle("funny-page-sheet--home", isHomePage);
				mainContentWrapper.classList.toggle("funny-page-sheet--inner", !isHomePage);
				mainContentWrapper.style.zIndex = "30";
				if (isMobile) {
					mainContentWrapper.style.position = "relative";
					mainContentWrapper.style.setProperty("top", "auto", "important");
	...
	private extendPageHeight(hide: boolean): void {
		...
		const isFullscreenMode = document.body.classList.contains("fullscreen-banner");
		if (!isBannerMode || isFullscreenMode) {
			return;
		}
